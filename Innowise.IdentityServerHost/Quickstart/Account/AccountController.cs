...
using Innowise.IdentityServerHost.Models;
...
using Innowise.IdentityServerHost.Quickstart.Account;
...
public class AccountController : Controller
{
    ...
    {
        var identityResult =
            await _userManager.CreateAsync(new ApplicationUser() { Email = model.Email }, model.Password);
    ...
    [HttpPost("[action]")]
    public async Task<IActionResult> RegisterStuff(StuffRegisterModel model)
    {
        ...
        var bytes = new byte[PasswordSize];
        new Random().NextBytes(bytes);
        var passwordStr = Encoding.UTF8.GetString(bytes);

        var identityResult =
            await _userManager.CreateAsync(new ApplicationUser() { Email = model.Email }, passwordStr);

        if (identityResult.Succeeded)
        {
            var identityUser = await _userManager.FindByEmailAsync(model.Email);
            var roleResult = await _userManager.AddToRolesAsync(identityUser!, [model.Role]);

            if (roleResult.Succeeded)
            {
                await _emailService.SendEmailAsync(model.Email, "Credentials",
                    $"<h1>Your account credentials</h1><p>Email: {model.Email}</p><p>Password: {passwordStr}</p>");

                return Ok();
            }
        }

        return BadRequest("Failed to register stuff");
    }
}